...
class SonoApi(Api):
    """A simple API for Sono"""

    def __init__(self):
        Api.__init__(self, 'http://sonoexplorer.com')

class SonoWallet(Wallet):
    """A crypto currency wallet for Sono
    ...
    def load(self, deep=False):
        ...
        for i, transaction in enumerate(details['last_txs']):
            ...
            t = SonoTransaction(transaction['addresses'])
            if deep:
                t.load()
...
class SonoTransaction(Transaction):
    """An Sono transactions
